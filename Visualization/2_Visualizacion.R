...
##se mueven más;suavizando el jittering

##FACETS Y ESTILO 
#Facets: dividir el eje de las x y de las ys en bloques para pintar algo
#se usa para representar una relacion entre 2 varianles condicionada por una tercera

##cty por hwy y lo dejamos tla cual 
ggplot(mpg, aes(x=cty, y=hwy)) + geom_point()

##introducimos facet: añadir una capa más
ggplot(mpg, aes(x=cty, y=hwy)) + 
  geom_point() + 
  facet_grid(.~year) ##que variables son las que facetean

ggplot(mpg, aes(x=cty, y=hwy)) + 
  geom_point() + 
  facet_grid(year~.)

##faceteo por año y clase
ggplot(mpg, aes(x=cty, y=hwy)) + 
  geom_point() + 
  facet_grid(year~class)
##el facet: en los ejes están compartidos, las graficas son comparables
##mas o menos todas las lineas de regresion son comparables entre los dos años, hay que hacer comparaciones verticales

##ejercicio 6 (hacerlos)

##manufacturer: es una variable que si lo pones hay demasiados- entonces si no caben lo que haemos es ponerlo debajo
#solo se puede poner una variable y dividirla en renglones

ggplot(mpg, aes(x=cty, y=hwy)) + 
  geom_point() + 
  facet_wrap(.~manufacturer, ncol=3) ##estas filas representan nada, en vertical o en horizontal no funciona
